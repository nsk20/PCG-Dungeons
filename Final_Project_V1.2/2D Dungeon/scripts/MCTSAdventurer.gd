...
var move_count_label: Label  # Variable for the text display
var best_move: int = 0
...
func _ready():
	...
	# Create Text Display Output 
	create_move_count_label()

func create_activate_button():
	activate_button = Button.new()
	activate_button.text = "MCTS Feedback"
	# Change Position of button
	activate_button.position = Vector2(400, -300)  
	activate_button.connect("pressed", Callable(self, "_on_activate_button_pressed"))
	add_child(activate_button)

func create_move_count_label():
	move_count_label = Label.new()
	move_count_label.position = Vector2(400, -250)  # Position it below the button
	move_count_label.text = "Moves: 0"
	add_child(move_count_label)

func update_move_count_display():
	if move_count_label:
		print(total_moves)
		move_count_label.text = "Best Moves: " + str(best_move)
	else:
		print("Warning: move_count_label is null")

func _on_activate_button_pressed():
	# To prevent rapid button pressing (cool down of 0.5 seconds; see below)
...
func _process(delta):
	if not is_active:
		...
		is_active = !is_active
		update_button_text()
		# Reset Play Variables to get new feedback
	...
	update_move_count_display()
	
	# Ensure the button stays in place
	if activate_button:
		activate_button.position = Vector2(400, -300)
	if move_count_label:
		move_count_label.position = Vector2(400, -250)
...
func reset_game():
	...
	map.unexplore()
	total_moves = 0
	#update_move_count_display()  # Reset the move count display
